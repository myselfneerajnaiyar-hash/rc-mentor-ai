...
export function internalPath(value) {
  ...
  if (url.origin !== "https://inbox.invalid" || url.pathname.startsWith("//") || new URL(decoded, "https://inbox.invalid").pathname.startsWith("//") || /^\/(?:api|_next)(?:\/|$)/.test(url.pathname)) throw new InboxInputError("actionUrl must be an internal path")
  return value
}
...
export function parseAction(body) {
  if (!body || typeof body !== "object" || Array.isArray(body) || Object.keys(body).some((key) => !["action", "ids", "all"].includes(key))) throw new InboxInputError("Invalid action payload")
  if (!["read", "unread", "archive", "delete", "restore", "unarchive"].includes(body.action)) throw new InboxInputError("Unsupported action")
  if (body.all === true) {
    if (body.action !== "read" || body.ids !== undefined) throw new InboxInputError("Only mark all read supports all")
  ...
  return { action: body.action, all: false, ids: [...new Set(body.ids.map(requireId))] }
}
