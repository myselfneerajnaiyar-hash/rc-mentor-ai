import { inboxQuery, withInboxDeadline, InboxTimeoutError } from "./request.js"
...
export async function unreadCount(db, userId, signal = AbortSignal.timeout(10000)) {
  const { count, error } = await inboxQuery(db.from(TABLE).select("id", { count: "exact", head: true }).eq("user_id", userId).is("read_at", null).is("archived_at", null).is("deleted_at", null), { signal })
...
export async function listInbox(db, userId, params) {
  ...
  const rows = result(await inboxQuery(query.order("priority", { ascending: false }).order("created_at", { ascending: false }).order("id", { ascending: false }).limit(pageSize + 1)))
...
export async function mutateInbox(db, userId, body, { signal = AbortSignal.timeout(40000) } = {}) {
  const { action, ids, all, ifUpdatedAt } = parseAction(body)
  const updated = []
  ...
  const bounded = (query) => {
    ...
    return inboxQuery(query, { signal })
  }
  try {
    for (let offset = 0; all || offset < ids.length; offset += 100) {
      ...
      if (ifUpdatedAt) query = query.eq("updated_at", ifUpdatedAt)
      if (action === "archive") query = query.is("archived_at", null)
      if (action === "unarchive") query = query.not("archived_at", "is", null)
      ...
      const response = await bounded(query.select("id,read_at,archived_at,deleted_at,updated_at"))
    ...
    if (!all) {
      for (let offset = 0; offset < ids.length; offset += 100) {
        current.push(...result(await bounded(db.from(TABLE).select("id,read_at,archived_at,deleted_at,updated_at").eq("user_id", userId).in("id", ids.slice(offset, offset + 100)))))
      }
...
export async function getInboxMessage(db, userId, id) {
  requireId(id)
  const { data, error } = await inboxQuery(db.from(TABLE).select(FIELDS).eq("user_id", userId).eq("id", id).maybeSingle())
  if (error) throw error
  return data
}
...
export function inboxHandlers({ db, authenticate, logger = console }) {
  const handle = (operation) => async (request, context) => {
    try {
      const identity = await withInboxDeadline(() => authenticate(request), { signal: request.signal, timeoutMs: 10000 })
      ...
      if (error instanceof InboxTimeoutError) return Response.json({ error: error.message }, { status: 504 })
  ...
  return {
    ...
    PATCH: handle(async (request, id) => mutateInbox(db, id, await withInboxDeadline(() => request.json(), { signal: request.signal, timeoutMs: 5000 }))),
