...
test("canonical trial activation enrolls through the existing welcome path", async () => {
  ...
  assert.match(source, /if \(whatsappOptIn\)/)
  assert.match(source, /whatsapp_opt_in: whatsappOptIn/)
  assert.match(source, /whatsapp_opt_in_at: whatsappOptInAt/)
  assert.match(source, /trial_days:\s*3/)
  assert.match(route, /!profile\.whatsapp_opt_in \|\| !profile\.whatsapp_opt_in_at/)
