...
import { COUPONS, normalizeCouponCode, validateCouponCode } from "@/lib/payments/pricing"
...
export async function findInfluencerCoupon(value) {
  ...
  if (!code || Object.hasOwn(COUPONS, code)) return null
...
export async function resolveCoupon(value) {
  const code = normalizeCouponCode(value)
  const influencerCoupon = await findInfluencerCoupon(code)
  return {
    ...validateCouponCode(code, influencerCoupon?.coupon),
    influencerCoupon,
  }
}
