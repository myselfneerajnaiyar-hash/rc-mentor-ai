...
const REQUIRED_SENDER_ENV = Object.freeze([
...
export function isWhatsAppSenderReady(env = process.env) {
  if (env.WHATSAPP_SENDER_READY !== "true") return false
  if (REQUIRED_SENDER_ENV.some((name) => !env[name]?.trim())) return false
  if (env.WHATSAPP_TEST_MODE === "true" && !env.WHATSAPP_TEST_PHONE?.trim()) return false
  return true
}
